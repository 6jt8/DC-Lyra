<p align="center">
  <a href="https://github.com/sayrox106/Lyra-MC">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=0:5865F2,100:e11d2e&height=200&section=header&text=Lyra&fontSize=80&fontAlignY=35&animation=twinkling&fontColor=fff" alt="Lyra" />
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/github/stars/sayrox106/Lyra-MC?style=flat&logo=github" alt="Stars">
  <img src="https://img.shields.io/github/forks/sayrox106/Lyra-MC?style=flat&logo=github" alt="Forks">
  <img src="https://img.shields.io/github/issues/sayrox106/Lyra-MC?style=flat&logo=github" alt="Issues">
  <img src="https://img.shields.io/github/last-commit/sayrox106/Lyra-MC?logo=git&logoColor=fff" alt="Last Commit">
  <img src="https://img.shields.io/badge/AGPL_3.0-FF4500?logo=gnu&logoColor=fff" alt="License">
</p>

<p align="center">
  <b>Your Discord Music Bot</b> — Modern · Modular · Type-Safe<br>
  Built with <a href="https://discord.js.org">discord.js v14</a>, <a href="https://riffy.js.org">Riffy</a> &amp; <a href="https://lavalink.dev">Lavalink</a>
</p>

<br>

<p align="center">
  <img src="https://i.imgur.com/6X35gly.gif" alt="Lyra Music Bot" width="800">
</p>

<br>

<h2 align="center">📦 Downloads</h2>

<div align="center">
<table>
<tr>
<td width="50%" align="center" valign="middle">
<img src="https://img.shields.io/badge/Stable-Release-2ea44f?style=for-the-badge" alt="Stable Release"><br><br>
<a href="https://github.com/sayrox106/Lyra-MC/releases/latest">
<img src="https://img.shields.io/badge/Download-.zip/.7z/.tar.gz-2ea44f?style=flat-square&v=1" alt="Download Stable">
</a>
</td>
<td width="50%" align="center" valign="middle">
<img src="https://img.shields.io/badge/Nightly-Dev-5865F2?style=for-the-badge" alt="Nightly"><br><br>
<a href="https://github.com/sayrox106/Lyra-MC/releases">
<img src="https://img.shields.io/badge/Nightly_Build-Auto_generated-5865F2?style=flat-square" alt="Nightly Build">
</a>
</td>
</tr>
</table>
</div>

<br>

<h2 align="center">✨ Features</h2>

<table align="center" width="100%">
  <tr>
    <td width="50%" valign="top" align="center" style="padding: 15px;">
      <h3>🎵 Music Playback</h3>
      <ul>
        <li>High-quality audio via Lavalink</li>
        <li>YouTube, Spotify, SoundCloud support</li>
        <li>Queue management (add, remove, skip, jump, move)</li>
        <li>24/7 mode in voice channels</li>
        <li>Autoplay & vote-skip system</li>
        <li>Volume control, seek, shuffle</li>
        <li>Audio filters (bassboost, nightcore, 3D, etc.)</li>
      </ul>
    </td>
    <td width="50%" valign="top" align="center" style="padding: 15px;">
      <h3>🎨 Player Experience</h3>
      <ul>
        <li>Live now-playing panel with buttons</li>
        <li>Synced lyrics display</li>
        <li>Generated song cards (Canvas)</li>
        <li>Progress bar & visualizer</li>
        <li>Multilingual UI (7 languages)</li>
        <li>Custom emoji system (auto-upload)</li>
        <li>Responsive status & activity</li>
      </ul>
    </td>
  </tr>
</table>
</div>

<br>

<h2 align="center">🚀 Quick Start</h2>

<table align="center" border="0">
<tr>
<td width="33%" align="center" valign="top">

<details>
<summary><b>🍞 Bun (Source)</b></summary>

```bash
unzip lyra-music-*-src.zip
cd lyra-music
bun install
cp .env.example .env
bun run start:bun
```

</details>

</td>
<td width="33%" align="center" valign="top">

<details>
<summary><b>🟢 Node.js (Dist)</b></summary>

```bash
unzip lyra-music-*-dist.zip
cd lyra-music
npm install
cp .env.example .env
npm start
```

</details>

</td>
<td width="33%" align="center" valign="top">

<details>
<summary><b>📦 From Source</b></summary>

```bash
git clone https://github.com/sayrox106/Lyra-MC.git
cd Lyra-MC
bun install   # or: npm install
bun run build # or: npm run build
bun start     # or: npm start
```

</details>

</td>
</tr>
</table>

<br>

<h2 align="center">🌐 Languages</h2>

<p align="center">
  🇬🇧 English · 🇩🇪 German · 🇫🇷 French · 🇪🇸 Spanish · 🇯🇵 Japanese · 🇰🇷 Korean · 🇷🇺 Russian
</p>

<br>

<h2 align="center">🛠️ Tech Stack</h2>

<p align="center">
  <img src="https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=fff" alt="TypeScript">
  <img src="https://img.shields.io/badge/discord.js_v14-5865F2?logo=discord&logoColor=fff" alt="discord.js">
  <img src="https://img.shields.io/badge/Lavalink-4A90D9?logo=cloudbees&logoColor=fff" alt="Lavalink">
  <img src="https://img.shields.io/badge/PostgreSQL-4169E1?logo=postgresql&logoColor=fff" alt="PostgreSQL">
  <img src="https://img.shields.io/badge/Node.js_20+-339933?logo=node.js&logoColor=fff" alt="Node.js">
  <img src="https://img.shields.io/badge/Bun-000000?logo=bun&logoColor=fff" alt="Bun">
  <img src="https://img.shields.io/badge/Express-000000?logo=express&logoColor=fff" alt="Express">
  <img src="https://img.shields.io/badge/Vitest-6E9F18?logo=vitest&logoColor=fff" alt="Vitest">
</p>

<br>

<h2 align="center">🤝 Contributors</h2>

<p align="center">
  <a href="https://github.com/sayrox106/Lyra-MC/blob/main/CONTRIBUTING.md">
    <img src="https://img.shields.io/badge/How_to_Contribute-5865F2?style=flat-square" alt="How to Contribute">
  </a>
</p>

<p align="center">
  <a href="https://github.com/sayrox106/Lyra-MC/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=sayrox106/Lyra-MC" alt="Contributors">
  </a>
</p>

<br>

<h2 align="center">📄 License</h2>

<p align="center">
  <b>AGPL-3.0</b> — see <a href="LICENSE">LICENSE</a><br>
  Copyright (c) 2026 <a href="https://github.com/sayrox106">sayrox106</a>
</p>

<br>

<p align="center">
  Built with ❤️ by <a href="https://github.com/sayrox106">sayrox106</a>
</p>
